##	Script to calculate the percentage if the truncation entails NMD. 2014.
...
while(my $line=<INPUT1>)
	{
		...
		if($line=~/^([^\t]+)\t([^\t]+)\t[^\t]+\t([^\t]+)\t([^\t]+)\t[^\t]+\t[^\t]+\t(.+)/)
		{
			...
			elsif($Effect=~/splice_acceptor_variant/ || $Effect=~/splice_donor_variant/)
			{
				...
				{
					#~ print"***********************$CHROM\t$SYMBOL\t$POS\t$REF\t$ALT\t$Effect\t$ENST\n";
...
foreach my $strand_tok(sort keys %{$hash3{$CHROM_tok}{$SYMBOL_tok}{$ENST_tok}})
{
			...
			{	
				foreach my $threshold_NMD_tok(sort keys %{$hash_EXON{$CHROM_tok}{$SYMBOL_tok}{$ENST_tok}{$strand_tok}{'THRESHOLD'}})
				{
					...
					if($threshold_NMD_tok eq 'NMD_negative')
					{
						...
						my $NMD="NMD_negative";
						print OUTPUT "$CHROM_tok\t$SYMBOL_tok\t$POS_tok\t$REF_tok\t$ALT_tok\t$Effect_tok\t$ENST_tok\t$NMD\n";	
					}
					...
					else
					{
						...
						{
							if($POS_tok < $threshold_NMD_tok)
							{
								my $NMD="NMD_positive";
								print OUTPUT "$CHROM_tok\t$SYMBOL_tok\t$POS_tok\t$REF_tok\t$ALT_tok\t$Effect_tok\t$ENST_tok\t$NMD\n";
							}
							elsif($POS_tok >= $threshold_NMD_tok)
							{
								my $NMD="NMD_negative";
								print OUTPUT "$CHROM_tok\t$SYMBOL_tok\t$POS_tok\t$REF_tok\t$ALT_tok\t$Effect_tok\t$ENST_tok\t$NMD\n";
							}
	...
	foreach my $Effect_tok(sort keys %{$hash_splice{$CHROM_tok}{$SYMBOL_tok}{$ENST_tok}{$POS_tok}{$REF_tok}{$ALT_tok}})
	{
		...
		if($POS_tok > $BEGIN_tmp[0] && $POS_tok < $END_tmp[scalar(@END_tmp)-1])
		{
			if($strand_tok eq '+')
			{	
				my @BEGIN_tmp_copy=@BEGIN_tmp;
				push(@BEGIN_tmp_copy,$POS_tok);
				my @BEGIN_tmp_copy_sorted=sort@BEGIN_tmp_copy;
				my ( $index )= grep { $BEGIN_tmp_copy_sorted[$_] eq $POS_tok }0 .. $#BEGIN_tmp_copy_sorted;
				my $POS_transferred=$BEGIN_tmp_copy_sorted[$index+1];
				#~ print "TRANSFER:$POS_tok->$POS_transferred\n";
				...
				foreach my $threshold_NMD_tok(sort keys %{$hash_EXON{$CHROM_tok}{$SYMBOL_tok}{$ENST_tok}{$strand_tok}{'THRESHOLD'}})
				{
					...
					else
					{
						if($END_tmp[scalar(@END_tmp)-1] > $threshold_NMD_tok)
						{
							if($POS_transferred < $threshold_NMD_tok)
							{
								my $NMD="NMD_positive";
								print OUTPUT "$CHROM_tok\t$SYMBOL_tok\t$POS_tok\t$REF_tok\t$ALT_tok\t$Effect_tok\t$ENST_tok\t$NMD\n";
							}
							elsif($POS_transferred >= $threshold_NMD_tok)
							{
								my $NMD="NMD_negative";
								print OUTPUT "$CHROM_tok\t$SYMBOL_tok\t$POS_tok\t$REF_tok\t$ALT_tok\t$Effect_tok\t$ENST_tok\t$NMD\n";
							}
			...
			{
				my @END_tmp_copy=@END_tmp;
				push(@END_tmp_copy,$POS_tok);
				my @END_tmp_copy_sorted=sort@END_tmp_copy;
				my ( $index )= grep { $END_tmp_copy_sorted[$_] eq $POS_tok }0 .. $#END_tmp_copy_sorted;
				my $POS_transferred=$END_tmp_copy_sorted[$index-1];
				#~ print "TRANSFER:$POS_tok->$POS_transferred\n";
