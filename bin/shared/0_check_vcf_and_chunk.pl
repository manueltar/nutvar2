...
# counter to chunk files

my $counter=0;

print "Select the number of chunks in which you want to divide the file"
my $number_of_chunks=<STDIN>;
chomp( $number_of_chunks);


for(my $l=1;$l <=$number_of_chunks;$l++)

if(open(INPUT, $input))# && open(OUTPUT, '>'.$output_chunked))
{
...
##	From lines carrying data we store every column in an array
			...
			{	
				# Here we start countig the lines to chunk the file
				...
				my $INFO="NaN";if (exists($HI0{"INFO"})){$INFO=".";}unless(defined($INFO)){$INFO="NaNein"; print "ERROR in INFO\n";}
...
}else {print "unable to open $output_chunked \n";}
